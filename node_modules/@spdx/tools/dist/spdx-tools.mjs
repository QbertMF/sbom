...
//
// SPDX-License-Identifier: MIT
class JsonHasExtractedLicensingInfos {
    extractedText;
    licenseId;
    comment;
    name;
    seeAlsos;
    constructor(extractedText, licenseId, comment, name, seeAlsos) {
    ...
    static fromOtherLicensingInfo(otherLicensingInfo) {
        return new JsonHasExtractedLicensingInfos(otherLicensingInfo.extractedText ?? "", otherLicensingInfo.licenseId ?? "SPDXRef-" + v4(), otherLicensingInfo.comment, otherLicensingInfo.licenseName?.toString(), otherLicensingInfo.crossReferences);
    }
}
...
class JsonDocument {
    ...
    constructor(spdxId, spdxVersion, name, documentNamespace, dataLicense, creationInfo, packages, files, relationships, hasExtractedLicensingInfos, externalDocumentRefs, comment) {
        this.SPDXID = spdxId;
    ...
    static fromDocument(document) {
        ...
        const jsonRelationships = document.relationships.length > 0
            ...
            : undefined;
        const jsonHasExtractedLicensingInfos = document.otherLicensingInfo.length > 0
            ? document.otherLicensingInfo.map((otherLicenseInfo) => JsonHasExtractedLicensingInfos.fromOtherLicensingInfo(otherLicenseInfo))
            : undefined;
...
class SPDXDocument extends Document {
    ...
    addOtherLicensingInformation(options) {
...
export { Actor, ActorType, Checksum, ChecksumAlgorithm, Document, DocumentCreationInfo, ExternalDocumentReference, ExternalPackageRef, ExternalPackageRefCategory, File, FileType, NOASSERTION, NONE, Package, PackagePurpose, Relationship, RelationshipType, SPDXDocument, SpdxNoAssertion, SpdxNone, createDocument };
